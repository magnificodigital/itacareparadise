...
function reactor_do_standard_header_titles() {
	...
	if ( is_page_template('page-templates/front-page.php') && $show_titles ) { ?>
	    ...
	    <?php }     
       echo '<span class="entry-date">'.the_date().'</span>';         
    ...
    elseif ( !get_post_format() && !is_page_template('page-templates/front-page.php') ) {  ?>    
		...
		<h2 class="entry-title"><?php the_title(); ?></h2>
	    <p class="entry-meta">Postado por <?php the_author(); ?> em <?php the_category(', '); ?></p>
...
function reactor_do_home_header_titles() {
...
add_action('reactor_post_header', 'reactor_do_standard_thumbnail', 2);
...
<div class="addthis_toolbox addthis_default_style" addthis:url="'.get_permalink().'">
