...
if ( !function_exists('reactor_slider') ) {
	function reactor_slider( $args = '' ) {
		...
		if ( $slider_query->have_posts() ) : 
            ...
            $output .= '<ul class="orbit-slides-container" data-orbit data-options="bullets:false;">';
			
            while ( $slider_query->have_posts() ) : $slider_query->the_post();
			    $post_id = get_the_ID();
				
				$s++;
                $output .= '<li data-orbit-slide="slide-'.$s.'">';
				...
				if ( has_excerpt() ) {
					...
					$output .= $slide_title;
					$output .= '<p class="slide-excerpt  hide-for-small">' . get_the_excerpt() . '</p>';
					$output .= '</div>';
				}
