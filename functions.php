...
function reactor_theme_setup() {
	...
	add_theme_support(
		'reactor-menus',
		array('top-bar-l', 'top-bar-r', 'main-menu', 'side-menu', 'footer-blog','footer-links', 'mobile_menu')
	);
	...
	add_theme_support(
		'reactor-post-types',
		array('slides','vantagens','depoimentos') // 'portfolio'
	);
...
function rem_custom_excerpt_more( $output ) {return preg_replace('/<a[^>]+>Continue reading.*?<\/a>/i','',$output);
}
add_filter( 'get_the_excerpt', 'rem_custom_excerpt_more', 20 );
